...
# Gerencia o sistema de skills do jogo
# Controla ativação, cooldowns e efeitos das skills
...
var skill_used: bool = false  # Flag para achievements

# Callbacks para efeitos das skills (serão definidos no Game.gd)
...
# Atualiza todos os timers de skills
...
# Ativa a skill de boost de dano
...
# Ativa a skill de boost de velocidade
...
# Ativa a skill de coletar moedas
...
# Verifica se uma skill está disponível (sem cooldown e não ativa)
...
# Retorna o cooldown restante de uma skill
...
# Retorna o tempo restante de uma skill ativa
...
# Define se tem o perk permanente de magnetismo
func set_coin_magnetism_perk(has_perk: bool) -> void:
	has_coin_magnetism_perk = has_perk
	if has_perk:
		# Desativar skill se perk permanente está ativo
		skill_magnetism_active = false
		skill_magnetism_time = 0.0

# Retorna o multiplicador de dano atual (considerando skill ativa)
func get_damage_multiplier() -> float:
	if skill_damage_boost_active:
		return GameConstants.SKILL_DAMAGE_BOOST_MULTIPLIER
	return 1.0

# Retorna o multiplicador de velocidade atual (considerando skill ativa)
func get_speed_multiplier() -> float:
	if skill_speed_boost_active:
		return GameConstants.SKILL_SPEED_BOOST_MULTIPLIER
	return 1.0

# Retorna o multiplicador de slow atual (considerando skill ativa)
func get_slow_multiplier() -> float:
	if skill_slow_all_active:
		return GameConstants.SKILL_SLOW_ALL_AMOUNT
	return 1.0

# Verifica se magnetismo está ativo (skill ou perk)
